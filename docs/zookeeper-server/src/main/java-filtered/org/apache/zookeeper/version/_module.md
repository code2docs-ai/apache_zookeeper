...
| 名称 | version |
| 编码语言 | .java |
| 代码路径 | zookeeper/zookeeper-server/src/main/java-filtered/org/apache/zookeeper/version |
| 包名 | zookeeper.docs.zookeeper-server.src.main.java-filtered.org.apache.zookeeper.version |
...
该流程图展示了Zookeeper项目中version目录下的文件结构关系。version作为父节点包含两个子文件：Info.java和VersionInfoMain.java，这两个文件都直接隶属于version目录，没有更深层次的嵌套结构。整个结构简洁明了，体现了版本信息相关文件的组织方式。

# 文件列表 File List

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [VersionInfoMain.java](VersionInfoMain.md) | file | 这是一个Java类，用于输出Apache ZooKeeper的版本信息和构建时间。 |
| [Info.java](Info.md) | file | 接口Info定义了版本信息，包含主版本号、次版本号、微版本号、限定符、修订号（已弃用）、修订哈希和构建日期。 |
